...
import { updatedataSetKickDto } from 'src/application/dto/dataSetKick';
...
@Injectable()
export class UpdateDataSetKickUseCase {
  constructor(
  ...
  async execute(
    ...
    try {
      const dataSetKick = await this.dataSetKickService.actualizarDataSetKick(
        id,
        dto,
      );
      this.eventEmitter.emit('dataSetKick.update', new DataSetKickEvent(dataSetKick));
